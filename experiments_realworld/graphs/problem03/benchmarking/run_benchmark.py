#!/usr/bin/env python3
"""
Benchmark runner for graphs/problem03 (CSES 1750 - Planets Queries I,
binary lifting on a functional graph).

Thin per-problem configuration: it only points the generic RealWorldBenchmark
template at this problem's files. The output is deterministic (one destination
vertex per query), so the engine's default exact-match correctness check is used
(no custom validator). All measurement methodology lives in the shared template
+ engine.
"""
import sys
import json
...
_BASE = Path(__file__).resolve().parents[1]          # .../graphs/problem03
...
def build_benchmark():
    return RealWorldBenchmark(
        problem_name='graphs/problem03 (CSES 1750 - Planets Queries I)',
        ...
        # Deterministic integer output -> engine default exact-match validator.
        validate_fn=None,
        traditional_time_limit=1.0,
        # CSES submission (optimal, 2026-05-30): Python TLE set vs C++ AC.
        critical_cases=[6, 7, 8, 9, 10, 12],
        control_cases=[1, 2, 3, 4, 5, 11, 13, 14],
    )
...
def main():
    parser = argparse.ArgumentParser(description='graphs/problem03 benchmark (CSES 1750)')
