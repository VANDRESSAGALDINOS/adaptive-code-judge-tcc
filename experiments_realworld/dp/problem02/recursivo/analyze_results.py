...
def analyze_results(input_dir, output_file):
    ...
    analysis = {
        ...
        "validation": {
            "total_executions": len(validation["test_results"]) * 3 * 3,  # casos x repetições x linguagens
            "cases_tested": list(validation["test_results"].keys()),
    ...
    for case, data in validation["test_results"].items():
    ...
    for case, data in validation["test_results"].items():
        python_times = [rep["python_traditional"]["time"] for rep in data["repetitions"] if rep["python_traditional"]["status"] == "ACCEPTED"]
